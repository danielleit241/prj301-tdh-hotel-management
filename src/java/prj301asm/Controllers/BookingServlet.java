...
public class BookingServlet extends HttpServlet {
    ...
    private void handleAdminActions(HttpServletRequest request, HttpServletResponse response, String action, String username)
        ...
        switch (action) {
            case "adminlist":
                request.setAttribute("roomBookings", bDao.getAdminBooking(request.getParameter("search")));
            ...
            case "createBooking":
                ...
                break;
            case "delete":
                deleteBooking(request, response, bDao);
            default:
    ...
    private void updateBooking(HttpServletRequest request, HttpServletResponse response, BookingDAO dao)
        ...
        if (roomIDStr != null) {
            ...
            if (dao.updateBooking(bookingID, phone, status, roomID)) {
                request.setAttribute("roomBookings", dao.getAdminBooking(null));
                forward(request, response, "manageBookings.jsp");
            }
        }
    }
    ...
    private void deleteBooking(HttpServletRequest request, HttpServletResponse response, BookingDAO bookingDao)
            throws ServletException, IOException {
        try {
            ...
            request.setAttribute("roomBookings", bookingDao.getAdminBooking(request.getParameter("search")));
            forward(request, response, "manageBookings.jsp");
        } catch (Exception e) {
